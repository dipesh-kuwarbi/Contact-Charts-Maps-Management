...
import { addContact, selectContacts, updateContact } from "../../store/slices/contactsSlice";
...
const ContactForm: React.FC<ContactFormProps> = ({ canEdit, handleSave, contactId }) => {
    ...
    if (Object.keys(formErrors).length > 0) {
        ...
        try {
          if (canEdit && contactId) {
            ...
            await dispatch(updateContact({ id: contactId, firstName, lastName, status }));
          } else {
            // Dispatch addContact action
            await dispatch(addContact({ id: Date.now(), firstName, lastName, status }));
          }
        ...
        <div className="bg-[#3E418D] p-8 rounded-lg shadow-lg hover:shadow-xl w-full max-w-lg transition-shadow duration-300">
          <form onSubmit={handleSubmit}>
            <div className="mt-4 flex items-center">
              <label className="text-white text-sm font-bold w-1/4" htmlFor="fName">
                First Name:
              </label>
              <input
            ...
            {errors.firstName && (
              <p className="text-red-500 text-xs italic mb-4">{errors.firstName}</p>
            )}

            <div className="mt-4 flex items-center">
              <label className="text-white text-sm font-bold w-1/4" htmlFor="lName">
                Last Name:
              </label>
              <input
                className={`w-full p-2 border ${
            ...
            {errors.lastName && (
              <p className="text-red-500 text-xs italic mb-4">{errors.lastName}</p>
            )}
            ...
            <div className="flex items-center justify-center mt-6">
              ...
              >
                {loading ? (
                  ...
                  canEdit ? "Save Contact" : "Create Contact"
                )}
              </button>
            </div>
          </form>
